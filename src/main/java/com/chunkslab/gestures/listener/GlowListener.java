...
public class GlowListener implements Listener {
    ...
    public void onGestureStart(GesturePlayAnimationEvent event) {
        plugin.getHookManager().getGlowManager().hideGlow(event.getGesturePlayer().getPlayer());
    }

    @EventHandler
    public void onGestureStop(GestureStopAnimationEvent event) {
        plugin.getHookManager().getGlowManager().showGlow(event.getGesturePlayer().getPlayer());
    }
