...
package com.chunkslab.gestures.playeranimator.nms.v1_18_R1.entity;
...
public class RendererImpl implements IRenderer {
    ...
    private List<Packet<?>> getSpawnPackets() {
        ...
        if (limb.getType() == LimbType.EFFECTS) {
            ...
            SoundEvent soundEvent = new SoundEvent(ResourceLocation.tryParse(soundEffect.getEffect()));
            Location location = limb.getModel().getBase().getLocation();
            return List.of(new ClientboundSoundPacket(soundEvent, SoundSource.MASTER, location.getX(), location.getY(), location.getZ(), soundEffect.getVolume(), soundEffect.getPitch()));
        }
