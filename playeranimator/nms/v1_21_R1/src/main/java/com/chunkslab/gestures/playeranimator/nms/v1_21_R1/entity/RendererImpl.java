...
package com.chunkslab.gestures.playeranimator.nms.v1_21_R1.entity;
...
public class RendererImpl implements IRenderer {
    ...
    private List<Packet<?>> getMovePackets() {
        ...
        buf.writeVarInt(cloud.getId());
        buf.writeDouble(finalLocation.getX());
        buf.writeDouble(finalLocation.getY());
        buf.writeDouble(finalLocation.getZ());
        buf.writeByte((byte) finalLocation.getYaw());
        buf.writeByte((byte) finalLocation.getPitch());
        buf.writeBoolean(false);
        ClientboundTeleportEntityPacket teleport = ClientboundTeleportEntityPacket.STREAM_CODEC.decode(buf);
