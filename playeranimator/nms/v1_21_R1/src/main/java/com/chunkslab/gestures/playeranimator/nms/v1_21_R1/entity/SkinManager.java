...
import com.chunkslab.gestures.playeranimator.nms.v1_21_R1.v1_21_R1;
...
public class SkinManager extends v1_21_R1 implements ISkinManager {
    ...
    public Object getSkinTexture(Player player) {
        ...
        byte[] bytes = Base64.getDecoder().decode(property.value());
        try {
    ...
    public boolean getSkinType(Player player) {
        ...
        try {
            TextureWrapper textureWrapper = TextureWrapper.fromBase64((gameProfile.getProperties().get("textures").iterator().next()).value());
