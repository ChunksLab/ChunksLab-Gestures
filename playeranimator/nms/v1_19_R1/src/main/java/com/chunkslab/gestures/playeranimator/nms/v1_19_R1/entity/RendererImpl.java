...
public class RendererImpl implements IRenderer {
    ...
    private List<Packet<?>> getSpawnPackets() {
        ...
        if (limb.getType() == LimbType.EFFECTS) {
            ...
            SoundEvent soundEvent = new SoundEvent(ResourceLocation.tryBuild(soundEffect.getNamespace(), soundEffect.getEffect()), 30.0f);
            Location location = limb.getModel().getBase().getLocation();
            return List.of(new ClientboundSoundPacket(soundEvent, SoundSource.MASTER, location.getX(), location.getY(), location.getZ(), soundEffect.getVolume(), soundEffect.getPitch(), 0));
        }
